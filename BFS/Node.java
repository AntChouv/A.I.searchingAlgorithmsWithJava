...
public class Node {
    ...
    public Node returnNextNode(){
        ...
        int min = h.findAndPrintMin(left);
        int posmin = 0;
        for(int i=0;i<=childs.size()-1;i++){
            int nums = 0;
            for (int j=0;j<=childs.get(i).left.length-1;j++){
                if (childs.get(i).left[j]!=0){
                    nums+=1;
                }
            }
            ...
            for (int j=0;j<=childs.get(i).left.length-1;j++){
                 if (childs.get(i).left[j]!=0){
                    ...
                    arr[pos] = childs.get(i).left[j];
                    pos+=1;
                }
            }
            
            childs.get(i).heuristicValue = h.findAndPrintMin(arr);
            MainBFS.nodesVisited+=1;
            if (min>childs.get(i).heuristicValue){
                min = childs.get(i).heuristicValue;
                posmin = i;
            }           
        }
        BFSsearch.open.add(childs.get(posmin));
        return childs.get(posmin);
